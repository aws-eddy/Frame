...
let is_symbol (c:char) = is_regexp (c.ToString()) @"[/$#!@&*-_{}|+]"
let is_bracket c = 
 match c with 
 | '<' -> false
 | _ -> is_letter c || is_whitespace c || is_digit c 
let is_anything c = is_letter c || is_whitespace c || is_digit c || is_symbol c
...
let panything : Parser<char> = psat is_anything
